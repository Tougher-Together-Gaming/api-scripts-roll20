...
const EASY_LIB_UTILITIES = (() => {
	...
	*******************************************************************************************************************/
	...
	// NOTE Vault implies cold storage like the Roll20 state; Conversly Forge implies runtime in-memory space.
	...
	*******************************************************************************************************************/
	...
	// NOTE Low Level functions are used by higher level functions.
	// They should not throw errors but instead degrade quietly.
	...
	function _getSharedForge() {
		...
		return typeof EASY_MODULE_FORGE !== "undefined" ? EASY_MODULE_FORGE : 1;
	}
	...
	function _logSyslogMessage({ severity, code, message }) {
		...
		const logMessage = `${getSyslogTimestamp()} [${this.modName}] (${severityMap[normalizedSeverity]}): {"code": ${code}, "message": "${message}"}`;
	...
	/**
	 * Clears a specific sub-node of the Roll20 API state or the entire state if no key is provided.
	 ...
	 * _purgeApiState({ key: 'EasyModuleVault' });
	 *
	 * // Clear the entire state
	 * _purgeApiState({});
	 *
	 * // Clear a specific sub-node without restarting
	 * _purgeApiState({ key: 'EasyModuleVault', restart: false });
	 *
	 * @returns {void}
	 */
	function _purgeApiState({ key, restart = false } = {}) {
		if (key) {
			if (state[key]) {
	...
	*******************************************************************************************************************/
	...
	// NOTE High Level functions are considered essential to the operation of a module.
	// They should attempt to fallback to defaults. If the outcome is still erroneous, throw an error to the Roll20 API.
	...
	function _createPhraseFactory() {
		const defaultLanguage = this.phraseLanguage;
		...
		const defaultPhrases = {
			...
			frFR: {
				...
				"0x026DAC9E": "Introuvable : Impossible de trouver '{{aRequestedFunc}}' disponible depuis {{globalName}}.",
		...
		function getPlayerLanguage(playerId) {
			...
			const playerPreferedLanguage = vault[phraseConfigKey]?.playerPreferedLanguage || {};

			return playerPreferedLanguage[playerId] || defaultLanguage;
		}
 ...
 */
	...
	function _createTemplateFactory() {
		...
		const defaultTemplateMap = {
			...
			"chatAlert": ({ title, description, command, remark }) => {
				return `[
				{
					"element": "div",
					"props": {
		...
		const templates = {}; // Internal storage for localized phrases
		...
		return {
			...
			get: ({ template, content = {} } = {}) => {
				...
				const templateString = templateGenerator(content);

				// Convert the template to a single-line string for easy integration
				return _convertToSingleLine({ multiline: templateString });
			},
			 ...
			 *
			 * @param {Object} options - The options object.
			 * @param {Object<string, Function>} options.newMap - A map of template names to template functions.
			 */
			set: ({ newMap }) => {
				Object.keys(templates).forEach(key => { return delete templates[key]; });
				Object.assign(templates, newMap);
			 ...
			 * Adds or updates specific templates in the factory.
			 *
			 * New templates must be provided as functions, which will generate dynamic content.
			 *
			 * @param {Object} options - The options object.
			 * @param {Object<string, Function>} options.newTemplates - The new templates to add or update.
			 * @throws {Error} If any template is not a function.
			 */
			add: ({ newTemplates }) => {
				if (typeof newTemplates !== "object" || newTemplates === null) {
				...
				Object.entries(newTemplates).forEach(([name, templateFunction]) => {
	 ...
	 */
	async function _whisperAlertMessage({ from, to, severity, title, description, remark }) {
		...
		try {
			...
			// _whisperPlayerMessage({ from, to, message }
			_whisperPlayerMessage({
				from: `${from}`,
				to: `${to}`,
	...
	*******************************************************************************************************************/
	...
	const checkInstall = () => {
		try {
			...
			phraseFactoryInit.init(this.phraseLanguage);
